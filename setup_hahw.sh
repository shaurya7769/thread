...
echo "---------------------------------------------------------"
echo "  0. Cleaning up existing services                       "
echo "---------------------------------------------------------"
sudo pkill -9 sensor_service || true
echo "Service port 5060 cleared."
...
echo "---------------------------------------------------------"
cd sensor_board
rm -f sensor_service src/*.o pru/*.obj pru/*.out
gcc -Wall -O2 -pthread -D_DEFAULT_SOURCE -c src/tcp_server.c -o src/tcp_server.o
gcc -Wall -O2 -pthread -D_DEFAULT_SOURCE -c src/acquisition.c -o src/acquisition.o
gcc -Wall -O2 -pthread -D_DEFAULT_SOURCE -o sensor_service src/tcp_server.o src/acquisition.o -lpthread -lrt -lm

# PRU Firmware Build
echo "Building PRU firmware..."
clpru -I. -I/usr/share/ti/cgt-pru/include -I/usr/lib/ti/pru-software-support-package/include -I/usr/lib/ti/pru-software-support-package/include/am335x -v3 -O2 -c pru/encoder_pru.c --obj_directory=pru/
...
APN="airtelgprs.com"
echo "Initializing modem for Airtel (APN: $APN)..."
...
if command -v nmcli >/dev/null 2>&1; then
    ...
    sudo nmcli con up Airtel 2>/dev/null || echo "[WARN] nmcli could not bring up modem immediately. Checking state..."
else
    echo "nmcli missing. Ensure NetworkManager is installed."
fi
...
ip addr show | grep -E "ppp|wwan|eth" || echo "[WARN] No active cellular/network interface found."
